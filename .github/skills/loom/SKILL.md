...
4. **Read before writing.** `loom show <id>` for prior attempts and their lessons — read every lesson before repeating work that already failed. `loom context <id>` for the hydration manifest (exact files to read, nothing more). Read whatever `knowledge/` files the manifest cites first: they carry what earlier tasks already paid to learn.
5. **Probe if needed.** If `loom show <id>` reports no acceptance tests (`needs_probe`), the task isn't executable yet. Write the failing acceptance test(s) that define "done," then `loom probe-done <id> --accept <path>... [--context <path>...]`. Pass the `knowledge/` files that bear on the task via `--context` alongside the source files, so whoever leases it next hydrates from what the fleet already knows instead of rediscovering it. Commit **and push** the failing tests + task file before continuing — `probe-done` only writes locally, and other agents' `loom next`/`show`/`context` read the canonical graph on `<remote>/main`, not your worktree.
...
9. **Write down what outlives the task.**
   Before closing out, record every durable fact you learned as `knowledge/<topic>.md`: one small file per topic, stating the fact and how to act on it.
   Attempt lessons live in per-task refs that only `loom show <id>` surfaces, so a lesson stays invisible to every other task until it lands in `knowledge/`.
   Durable means the next agent would otherwise pay to re-derive it: a constraint the code enforces, a workaround a tool needs, a decision and its reason.
   Append to the existing topic file when one already covers the ground, rather than opening a near-duplicate.
...
- Seed new tasks with the `knowledge/` files that bear on them via `--context` at `loom task-create` time, the same way a probe does. `loom retro` reports which knowledge files exist and which its candidates would create.
- To refine `deps`, `goal`, or `value`, settle them before `task-create` runs — nothing edits those fields afterward. A correction post-creation means `loom dead <id> --reason "..."` and recreating under a fresh id (cheap before any lease or attempt exists, but recreate any dependent whose `--dep` still points at the old id, since a dep only clears on `done`, not `dead`). To refine acceptance criteria, call `loom probe-done <id> --accept ...` again — it carries no lease or state check, so it's a genuine do-over.
